...
> （原仓库根的 `CODEX.md` 已于 2026-09-19 删除，其简版硬规则职责已并入本文件。）
>
> 下面是概览。
...
| 网站导航 | `/nav/` 按用途分组的站点收藏夹（毛玻璃卡片网格）：**6 组共 21 个站点**，游戏相关刻意收在最后一组。条目在 `site.ts` 的 `SITE_NAV`；`href` 以 `#` 开头的条目渲染成虚线**空位卡**（不可点击） |
...
- **网站导航**：改 `site.ts` 的 `SITE_NAV` —— 往对应分组加一行（`name`/`href`/`desc`/`tag`），
  `href` 写成完整地址即启用；若地址还没定，可写成 `#` 开头（页面渲染成虚线空位卡，不可点击）。
  **数组顺序即页面顺序**（游戏相关目前排最后），分组删空则该组不渲染。

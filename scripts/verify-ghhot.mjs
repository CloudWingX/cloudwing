// GitHub 热榜验收（§56，2026-09-20 站长要求；§69 数据源改造 2026-09-22）：右栏站点统计卡上方的热榜卡片。
//
// 站长要求：
//   ① 位置：右侧栏站点统计卡片上方；
//   ② 日/周/月/年四个子选项，默认每日；
...
//   ④ 点击跳转对应仓库。
//   ⑤（§69）数据源 = 同域构建快照 /api/gh-hot.json —— 访客端不再直连 api.github.com，
//      无鉴权限流与跨境网络问题从根上消除；本脚本可放心重复跑。
//
...
/* ② 同域快照就位且结构完整（§69）：前端唯一数据源，四周期缺一不可 */
const snap = await fetch(`${BASE}/api/gh-hot.json`).then((r) => r.json());
const sp = snap && snap.periods;
check('站内快照 /api/gh-hot.json 存在且含四周期',
  !!sp && ['daily', 'weekly', 'monthly', 'yearly'].every((k) => Array.isArray(sp[k]?.items) && sp[k].items.length > 0),
  sp ? Object.keys(sp).join(',') : 'missing');
...
/* ④ 切每周：tab 选中态更新 + 渲染（快照已全周期写缓存，直接命中零请求） */

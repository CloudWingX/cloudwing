...
>
> **容器宽度只有一处真值**：`global.css` 的 `--w-max`（2026-09-18 二轮定为 **1300**，
> 即"容器盒宽"，内容净宽 = 1300 − 52×2 = 1196）。`Header.astro` 的
> `.header-container` 必须同步成同一个值（它写字面 px，滚动收缩态 1140）。
> 详情与踩坑见 `docs/HANDOFF.md` **§36.1**。
...
6. **推送后要轮询线上确认**：CF 构建约 30~90 秒（2026-09-18 四轮实测 **38 秒**）。
   **不要用 chunk 哈希判断部署**（本地与线上哈希可能不同），用**内容特征**去线上 HTML/CSS 里找。
...
7. **新写像素/几何断言前读 HANDOFF §7.6**（三条硬要求）。本项目已经出过两次
   ...
   以及只扫容器外面、绕开容器边缘的扫描盲区。**只看到绿灯不算验证过 —— 要证明它在缺陷态下会红。**
8. **别用 PowerShell 读写带中文的源文件**（GBK 往返会把整file 变乱码，HANDOFF §7.7）：

...
> ⚡ **压缩版交接入口（2026-09-23 起）**：`docs/handover/HANDOVER.md` ——
> 按《交接文档压缩方案 v1.0》整理的 C0–C3 分级版（契约无损/操作 checklist/设计摘要/
> 历史索引），**新接手者从那里开始读**；本文件转为「完整历史日志 + 常驻手册」继续维护，
> 每轮交付照旧在此追加编号章节，并定期重打归档（`docs/handover/archive/`）。
>
...
>
> **最后更新（2026-09-25 晚）**
> - **§83 手机端重构系列**（小米浏览器反馈驱动，全部手机端、桌面零影响）：
>   §83 排版（影像两列，全页滚动深度 4565→3516px）、§83.1/§83.2 显现隐形根治
>   （rAF 冻结 + 慢网「JS 包迟到」双保险）、§83.3 音乐页封面 SSR、§83.4 视频手机端不加载
>   （省 5.92MB）+ 字体异步 + modulepreload 22 页、§83.5 背景统一 + 黑边根治 + 删 5 个零引用死代码。
> - **§82 访问速度诊断与优化**：分级方案 A1 视频瘦身 15.55→5.92MB（PSNR 40.4–41.0dB）、
>   B1 绑定主域 **cloudwing.top**（已上线）。
> - **推送体系升级**：SSH over 443 主路 + `gh-push.sh` 一键三策略（见 §2）。
>
> **更早的轮次摘要（2026-09-23 及以前，保留索引）**：
> §80 移动端导航改形 StaggeredMenu 抽屉（四轮迭代）｜§79 音乐页官方封面 + 同步歌词｜
> §66 全站统一 1300 网格｜§65 Peak 图集｜§64 日历子页面｜§57 章节地图与阅读路径｜
> §56 右栏 GitHub 热榜卡｜§55 归档页更新热力图｜§54 「记录」只开合二级菜单｜
> §53 「记录」三页 + 档案页壳层统一｜§52 子页面壳层统一｜§51 「记录」悬停下拉｜
> §50 日志子页面｜§49 交互策略｜§48 导航卡真实图标｜§43 删 CODEX.md｜§41 极简化｜
> §40 作品库下线｜§37 React #424 根治｜§27 亮色主题删除。
> （2026-09-24 早：verify-music 入批跑队列，队列 23→24；补齐 HANDOVER.md 压缩入口。）
> 同日凌晨 **壳层统一：子页面页头格式统一（kicker「EN / 中文」、删标题下描述）、
> 部署：Cloudflare 自动构建 —— 推 `main` → 构建 `npm run build` → 线上主域 <https://cloudwing.top>
...
> 单入口 `node scripts/smoke.mjs` 为 **76/76**（2026-09-25 晚基线）。
> ⚠️ 沙箱限制：node 派生子进程被禁，全量需 bash for 循环逐个顶层调用（见 §14）。
...
| **全站背景视频** | 5.92MB mp4 自托管（不走外链，2026-09-25 瘦身后，§82 A1）＋ 上浅下深渐变遮罩 ＋ poster 降级；`transition:persist` 防软导航重建 | §23 |
...
| GitHub 直连时通时断 | ★**2026-09-25 起有治本方案**：push URL 已切 **SSH over 443**（`ssh.github.com:443`，SSH 无 SNI 可掐，HTTPS 被干扰时照常能推）——密钥 `~/.ssh/id_ed25519`（公钥已加 GitHub 账号），路由配置 `~/.ssh/github_config`（仅本仓库经 `core.sshCommand` 引用，不碰用户原 `.ssh/config`）。**一键推送：`bash "D:/deep seek workplace/.workbuddy/gh-push.sh" [分支]`**——SSH 优先 → HTTPS 直连 → TCP 隧道（自动探测可达 IP）三策略 + remote ref 自动核验 |
| ★推送后凭据 helper 被静默关闭的坑（2026-09-25 实测）★ | 一次失败重试触发 PortableGit 的 `helper-selector`，它把 `~/.gitconfig` 写成 `credential.helper=`（空值=禁用一切 helper）+ `selected=<no helper>`，之后**所有推送 401**（报 could not read Username）。修复：`.gitconfig` 设 `helper = !"…git-credential-manager.exe"`（凭据库里已有 `git:https://github.com` 存量，免交互）。若再遇「左上角弹窗选 manager」，选一次即可，selector 会写入 GCM 完整路径 |
| GitHub 直连时通时断 | ~~推送失败就重试~~（旧口径：HTTPS 重试/隧道，现降级为 gh-push.sh 的兜底策略） |
...
| 脚本 | 覆盖什么 | 当前基线 |
|---|---|---|
| `smoke.mjs` | ★总入口：各页横向溢出/控制台异常/侧栏存在与等高、长页两栏逐帧同步、"不钉在导航栏下"、软导航往返后的文章列表/详情跳转与热力图浮层交互、**软导航一圈后画廊大图开/关（§58）+ 画廊文件夹墙/进图集/返回（§59）**、手机端侧栏隐藏（含 `/log/`） | **76/76**（2026-09-23 晚，§80 抽屉系列复跑全绿；音乐页段 +9 见 §79） |
...
| `verify-home.mjs` | 首页四层结构（最新**博客**）、图片策略、防 CLS 容器、alt、区块间距、SEO（6 页）、移动端（作品单列 + **影像两列**，§83） | **26/26**（2026-09-25 影像两列断言同步） |
...
| 背景视频体积 | ~~嫌重可以压~~ **已完成（2026-09-25，§82 A1）**：15.55MB → **5.92MB**（h264 crf27 maxrate2M 重编码 + faststart，PSNR 40.4-41.0dB 三帧核验，无音轨不变、分辨率不变）；见 §23 的 `VIDEO_BG` 令牌 |
...
## §77 Peak 图集 +10（2026-09-22 深夜）

- 需求：站长提供 10 张 PEAK 截图（AppID 3527290，Steam 原生 JPG 1920×1080，
  298–365KB 与库内既有 peak 同规格）→ **原图直存**（不转 webp，与 peak-001~018
  一致），按文件名时间戳升序接续编号 `peak-019 ~ peak-028`（内容为拍立得
  大头贴与游玩瞬间，date 全部 2026-09-22）。
- changelog：+1 条「Peak 图集新增 10 张截图」（18 → 28 张，总数 122 → 132）。
  ⚠️ 复用 §74 的独立 .mjs 追加法时，模板字符串末尾混入一个 `;`（引号外），
  YAML 语义脏数据 —— 已修；**changelog 模板生成后必须打印尾部复核**。
- 环境备忘：隔数小时后 9222 无头 Edge 与 preview 双双退出 —— 按静音铁律标准
  命令重启 Edge（profile cwcdp-mute）；preview 用 Bash run_in_background 起
  （spawn detached 随父进程被杀，不可用；curl 走系统代理会假报 502/404，
  以 node net.connect 直测端口为准）。
- 回归：构建产物校验（dist/shots/peak 28 张、gallery meta「28 张 · 最近
  2026-09-22」）+ smoke **76/76**。
...
- **§80 补 2（同晚）**：站长「宽度再缩一半、右侧不要留白、低可见度磨砂」——
  宽度 clamp(170,38vw,200)、编号改内联 flex 跟排（右内边距 0）、
...
## §82 访问速度诊断与优化方案（2026-09-25，站长裁决：暂不动手，仅留档）

> 站长反馈"网页访问很慢，是不是部署在 GitHub 的原因"。结论：**不是**——GitHub 只是
> push 目标与 CI 触发源，访客流量全部打在 Cloudflare 边缘。实测探针：
> `_shots/probe-access-speed.mjs`（工作区 _shots，本机大陆网络视角，只读）。

**实测数据（2026-09-25）**

| 探测项 | 实测值 |
|---|---|
| CF 边缘 | cf-ray = **LAX（美西）**——免费版大陆无 PoP，绕美西是 TTFB 物理下限 |
| 首页 HTML | 冷 TTFB **925ms** / 热 236ms，38KB |
| CSS chunk | 435ms，83KB，**~400KB/s**（CF 免费版到大陆典型劣化带宽） |
| 背景视频前 1MB | 2594ms，**~460KB/s** → 15.5MB 全量 ≈ **34 秒**（autoplay，体感最大单项） |
| Google Fonts | 本机实测可达（~230ms），但大陆整体属不稳定外链 |
| 其余资源 | HTML 38KB / CSS 83KB / JS 为分包小件——优化空间不在这里 |

**分级方案（未执行，等站长点单）**
- **A1 视频瘦身**：`bg-loop.mp4` 15.5MB → ≤8MB（降码率重编码）；可选移动端 poster 替代视频
  —— **✅ 已完成（2026-09-25）**：crf27/maxrate2k/veryfast + `+faststart`，产物 **5.92MB**（-62%），
  PSNR 三帧 40.4-41.0dB（视觉无损档）、通道均值偏差 ≤0.6；本地 verify-videobg 23/23 + smoke 76/76；
  提交 `a08d540`，线上 content-length 6206107 已核验。faststart 让 moov 前置，边下边播。
- **A3 加载时序**：poster 即显 + 延迟起播；React 岛 `client:idle`（⚠️ 禁 `client:visible`，§6.19 #424）
- **A2 自托管 Inter 字体**（~46KB woff2 + font-display:swap）——⚠️ §13.1 被叫停过的方案，
  因"访客在大陆"背景重提；**须站长点头才做**
- **B1 绑自定义域名**：绕开 pages.dev 的 DNS 污染/SNI 干扰（可达性↑，TTFB 不变）；需域名，备案非必须
  —— **✅ 主域已完成（2026-09-25）**：`cloudwing.top`（腾讯云注册）NS 已切 CF
...
- **B2 双线托管**：镜像部署腾讯 EdgeOne Pages（默认域名大陆可达性显著优于 pages.dev，
  自定义域才要备案）——大陆 TTFB 的实质解；需评估 §60/§63 构建指纹自愈对双源的兼容
- **B3 缓存策略**：`/_astro/*`（带 hash）`Cache-Control: immutable`，回访秒开
- C 级（不推荐）：国内 CDN 需备案；CF China Network 需企业版

> 优先级建议：A1（体感最大单项）→ B1（有域名就做，改动为零）→ A3 → A2/B2 待定。

## §83 手机端首页排版优化（2026-09-25）

> 站长指令「优化网页手机端首页排版」。取证先行：`_shots/mobile-home-audit.mjs` /
> `audit2.mjs`（375/390/414 三视口全页截图 + 版式量测，本机 390 实测）。改动全部限 ≤768/≤620 移动端，桌面零变化。

**改了什么**
1. **精选影像 ≤620 改两列**（原单列大图 ×6，影像区 1630px 高）：`.shot-grid` → `repeat(2,minmax(0,1fr))` +
   gap 10px，`.shot-meta` 收敛（0.66rem / gap .35rem，仍常显）。影像区 **1630 → 605px**，
   首页总高 **4565 → 3516px**（省 1049px 滚动深度）。画廊页维持单列不变（§9.1 口径）。
2. **hero 底部 padding** ≤768：`96px 20px 64px` → `96px 20px 40px`（hero→「最新文章」空白 144→120px；
   `.sec` 80px 规范下限未动）。

**断言同步（§7.6 先红后绿）**：`verify-home.mjs` 移动端断言「移动端单列堆叠」→
「移动端作品单列 + 影像两列」——旧代码实测红（影像=350px），改后绿（作品=350px、影像=170px 170px），
**26/26**；smoke **76/76**；零横向溢出。提交 `43b5787`，线上以新 chunk 指纹 `index.BoEkLL6-`
poll-deploy 确认（⚠️ poll 特征别带冒号——MSYS 会把 `x:y` 当路径表转换，两次翻车；
且 Astro scoped 样式编译后是 `.shot-meta[data-astro-cid-…]{…}`，裸类名+属性串匹配不到）。

### §83.1 手机端「刷新后代码卡片下方内容消失」修复（2026-09-25 晚）

> 站长真机反馈：刷新后代码卡片下方内容消失、过一会儿才出现；本机 CDP（390×844，
> 本地+线上，含 reload 恢复滚动位复现）**无法复现**（刷新后 unrevealed=0、零异常）。

- **根因判定**：手机浏览器息屏/切后台时 **rAF 冻结**——`scanReveals` 的 sweep 兜底走
  `requestAnimationFrame` 节流，被冻结时未显现元素停在 `opacity:0`，回前台才恢复，
  完全吻合「消失→过会儿有了」。
- **修复（`ui.js` scanReveals）**：补两道与 rAF 无关的驱动——
  ① `pageshow`（bfcache 恢复）与 `visibilitychange`（切后台回前台）直接同步 sweep；
  ② **2.5s 超时强制显现**（`__rvForceTimer` 单例）：入场动画远早于此完成，
  最坏情况元素只隐藏 2.5s，杜绝任何环境「永久隐藏」。

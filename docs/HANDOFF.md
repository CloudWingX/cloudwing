...
>
> 最后更新：2026-09-20 傍晚 **交接完善：全篇基线数字刷新（21 脚本 / 21/21 / smoke 58/58）、
...
> 此前 20/20 见 §53 / §54）；
> 单入口 `node scripts/smoke.mjs` 为 **58/58**（2026-09-20 实测）。
...
> **接手第一件事**：`cd endfield-blog && npm run build && node scripts/smoke.mjs`（期望 **58/58**）。
...
| `verify-ghhot.mjs` | ★**GitHub 热榜卡断言**（§56）：① 卡片位于 STATS 卡正上方；② 四 tab（日/周/月/年）+ 默认每日；③ 每日榜真实渲染 ≥5 条；④ 条目链接跳转 github.com 仓库；⑤ 条目带 star；⑥ 切每周正常渲染；⑦ 日/周**缓存桶 = 当天/本周一**（「24 点自动换新」的实现依据）。⚠️ 真实调用 api.github.com（无鉴权搜索限流 10 次/分），只实拉日/周两周期，**别在循环里跑** | **9/9** |
...
node scripts/smoke.mjs https://cloudwing.pages.dev                 # 期望 58/58（#424 已修复，见 §37）
...
| 2 | §1 五分钟上手 → 照做一遍 build + smoke（期望 **58/58**） | 把本地环境跑起来 |

...
>
> 最后更新：2026-09-18 四轮（删掉首页最后一层"只铺在容器盒里"的装饰薄膜，
...
13 个验证脚本 + 2 个诊断脚本全绿、可读性 0 处不达标、7 个页面 0 JS 异常、手机端 0 横向溢出。
...
| `verify-hero.mjs` | 首页 Hero：几何/比例 + 与主栅格对齐（同宽/同内边距/内容左缘重合）+ 两行标题 + 强调色预设 + **整块竖向居中/容器居中/标题字号/`--card-shift`** | **85/85** |
| `verify-home.mjs` | 首页四层结构、图片策略、防 CLS 容器、alt、区块间距、SEO、移动端单列 | **23/23** |
| `verify-nav-shrink.mjs` | 导航三态（贴顶通栏/滚动胶囊/回顶）+ 形态过渡 + 移动端汉堡菜单 + **导航容器与主内容容器同宽、同左缘、自身居中** | **39/39** |
...
| **线上间歇 `React error #424`** | 仅线上、间歇复现，本地（含改动前的基线）连跑 10+ 轮全绿；缓存、部署完整性、本次改动均已排除（详见 §32）。**尚未定位根因**。⚠️ 2026-09-18 四轮又测一组：线上 `smoke` 3 轮 = **43/45 / 45/45 / 42/45**（失败项都只有 #424），同一时刻本地 45/45；四轮改动是**纯 CSS/纯删除、没碰任何 React 岛** → 确认与本轮无关。要不要我专门开一轮来定位它？ |

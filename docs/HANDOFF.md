...
>
> 最后更新：2026-09-18（导航 / 首页 Hero / 代码卡片 / 强调色预设 / 全局容器宽度整轮重做，
> HEAD `9375f6f`，已推送、线上已验证）；Cloudflare 自动构建部署。
...
> ⚠️ **改导航 / Hero / 强调色 / 容器宽度前先读 §33–§37**（形态取值与实测踩坑都在那里）。
...
> 改导航 / 首页 Hero / 强调色 / 容器宽度之前，**先读 §33–§37**（这四块在 2026-09-18 被连续重做过，
...
> HEAD `9375f6f`，全部已提交、已推送、线上已验证（1440 与 1920 均实测）。详细见 §33–§37。
...
| **全局容器加宽** | `--w-max` 1140 → 1428（内容净宽 **1324**，与参考一致）；hero 对齐同一栅格 | §37 |

**本轮的几条主线**（下次接手先看）：
1. **"参考"要当数据量，不要当感觉猜**：每次改前都先把参考站渲染出来量（几何/字号/容器宽），
   改完再逐项对比。§34–§37 里的数值全是这么来的。
...
34. **★强调色预设（色卡切换）★**（2026-09-18，用户要求"参考 reactbits 首页代码卡片下方
    ...
    - **自检**：`verify-hero.mjs`（68 项：另有色值胶囊、随预设变色、词法覆盖 ≥6 类、
      卡片已缩小、标题与卡片齐平）。
      想核对某个预设下的整体对比度：`$env:ACCENT='ember'; node scripts/contrast-audit.mjs <url> dark`
      —— 五套预设当前都是 **0 处不达标**。

36. **★首页元素比例照参考站重定★**（2026-09-18，用户要求"参考 reactbits 首页大标题字号、
...
37. **★全局主内容容器加宽到与参考站一致★**（2026-09-18，用户要求"内容向左右扩展、
...
| `verify-hero.mjs` | 首页 Hero：几何/比例 + 与主栅格对齐（同宽/同内边距/内容左缘重合）+ 两行标题 + 强调色预设 | **70/70** |
...
| `verify-videobg.mjs` | 背景视频：播放/层级/透明度/遮罩/hero 文字在视频上的**实际像素**对比度（用"隐藏文字的对照页"取样，并自检对照页版式与真实页一致） | **17/17** |
...
| 首页 Hero 右侧留白（1920） | 左栏 645 + gap 80 + 卡片 440 = 1165，用不满 1324 的内容宽，**右侧空约 159px**。要填满就得加宽左栏或卡片 —— 那属于"改内部组件尺寸"，本次按要求没动。要改说一声（§37） |
| 大标题字号 vs 参考 | 参考 h1 是 66px@645 栏；我们第一行 28 字符（每字号 13.19px），保两行只能到 **48px**。想要 66px 就把 `.ht-l1` 的 nowrap 去掉（会折成 3 行）（§36） |
...
- **补一条 `src/content/changelog/2026-09-18.md`**：这一轮改了导航、首页 Hero、代码卡片、
  色卡切换与全局容器宽度，侧栏「更新日历」目前完全看不到（§10 已列）；
- **定位线上间歇 `React error #424`**：唯一未解决的线上问题，证据与已排除项见 §32，
  本地（含改动前基线）复现不出来；
- **给文案加自动化断言**：`SITE.notice` / `tagline` 曾带着"浅色通透"上线而 14 个脚本全绿（§32），
  现在只有 §7.4.2 的手动清单；
- **首页 Hero 右侧留白**（1920 下 159px）与**代码卡片再瘦身**：都要动内部尺寸，需用户确认（§34/§37）；
...
# 4) 线上同一套（先确认推送成功、CF 构建完成）
git ls-remote origin main
node scripts/smoke.mjs https://cloudwing.pages.dev
```

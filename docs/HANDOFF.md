...
>
> 最后更新：2026-09-19 晚 **新增「日志」子页面 `/log/`——changelog 的完整展开版，见 §50**；
...
> 最近一次 **19/19 全绿**（2026-09-19 晚，第 19 号脚本 `verify-log` 加入后的发布前全量，见 §50.4；
...
验证脚本现为 19 个：发布前全量 **19/19**（2026-09-19 晚，见 §50.4；18 脚本时代 18/18 见 §49.4）；可读性 0 处不达标、
...
| 脚本 | 覆盖什么 | 当前基线 |
|---|---|---|
| `smoke.mjs` | ★总入口：各页横向溢出/控制台异常/侧栏存在与等高、长页两栏逐帧同步、"不钉在导航栏下"、软导航往返后的文章列表/详情跳转与热力图浮层交互、手机端侧栏隐藏（含 `/log/`） | **58/58**（2026-09-19 晚新增 `/log/` 后 53 → 58） |
...
| `verify-nav-shrink.mjs` | 导航三态（贴顶通栏/滚动胶囊/回顶）+ 形态过渡 + 移动端汉堡菜单（**含"抽屉已无分类入口"、"已无外链按钮"、"左右各内缩 12px"、"四角圆角 ≥12px 且一致"**、抽屉一级链接数随 NAV 增长同步 +1）+ **导航容器与主内容容器同宽、同左缘、自身居中** | **40/40**（2026-09-19 晚 `/log/` 后抽屉断言 7 → 8 修正，条数不变） |

...
>
> 最后更新：2026-09-19 下午 **新增「网站导航」子页面 `/nav/`（毛玻璃卡片网格，条目先占位，见 §44）**；
...
> 最近一次 **16/16 全绿**（单个入口 `node scripts/smoke.mjs` 为 **53/53**）。
...
> **接手第一件事**：`cd endfield-blog && npm run build && node scripts/smoke.mjs`（期望 **53/53**）。
...
| 脚本 | 覆盖什么 | 当前基线 |
|---|---|---|
| `smoke.mjs` | ★总入口：各页横向溢出/控制台异常/侧栏存在与等高、长页两栏逐帧同步、"不钉在导航栏下"、软导航往返后的文章列表/详情跳转与热力图浮层交互、手机端侧栏隐藏（6 个子页面） | **53/53**（2026-09-19 新增 `/nav/` 后 48 → 53） |
...
| `verify-home.mjs` | 首页四层结构（最新**博客**）、图片策略、防 CLS 容器、alt、区块间距、SEO（6 页）、移动端单列 | **25/25** |
...
| `verify-videobg-global.mjs` | 全站背景一致性：逐页硬刷新 + 软导航一圈，视频未被重建、旧背景仍在（含 `/nav/`） | **33/33** |
...
| `mobile-shots.mjs` | 三机型视口 × 7 页截图 + 横向溢出统计 | **21/21 无溢出** |
| `diag-errors.mjs` | 逐页 JS 异常计数 | 8 页全 **0** |
...
## 44. ★新增「网站导航」子页面 `/nav/`（毛玻璃卡片网格，条目先占位）（2026-09-19 下午）★
...
- **数据**：全部来自 `src/site.ts` 新增导出的 `SITE_NAV` —— **页面零硬编码**，
  ...
  四个分组：技术站点 / 工具与效率 / 素材与资源 / 友情链接，每组 3 个空位。
...
- 卡片直接复用全站令牌：`--glass` / `--glass-blur` / `--line-2` / `--r-l` / `--shadow-sm` /
  ...
  新页面不需要单独适配。
- ⚠️ 玻璃元素上 `-webkit-backdrop-filter` 必须写在标准属性**之前**（构建压缩会整条丢掉，§22）。
...
1. **把空位填上**：只改 `src/site.ts` 的 `SITE_NAV`（把 `href` 换成真实地址），推 `main` 即生效。
2. ⚠️ **空位卡目前对访客也是可见的**（虚线 + "空位"角标）。如果不想在补齐之前让访客看到骨架态，
   最小改法是在 `index.astro` 里过滤掉 `!it.live` 的条目，或 `live === 0` 时整块网格不渲染。
